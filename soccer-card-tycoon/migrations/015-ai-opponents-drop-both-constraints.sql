-- Complete Fix for AI Opponents - Drop All User Constraints
-- This removes foreign key constraints from both profiles and teams tables

-- Step 1: Drop the profiles_id_fkey constraint
DO $$
BEGIN
  IF EXISTS (
...
-- Step 2: Drop the teams_user_id_fkey constraint
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name = 'teams_user_id_fkey'
    AND table_name = 'teams'
  ) THEN
    ALTER TABLE teams DROP CONSTRAINT teams_user_id_fkey;
    RAISE NOTICE '✓ Dropped teams_user_id_fkey constraint';
  ELSE
    RAISE NOTICE 'Constraint teams_user_id_fkey does not exist';
  END IF;
END $$;

-- Step 3: Clean up any existing bot data
